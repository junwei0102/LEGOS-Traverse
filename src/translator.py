...
def parse_trace(trace_content):
    ...
    try:
        ...
        # 优先使用原始的完整规则文件temp_rules.sleec
        temp_rules_path = os.path.join(rules_dir, "temp_rules.sleec")
        generated_rules_path = os.path.join(rules_dir, "generated_rules.sleec")
        
        rules_file = temp_rules_path if os.path.exists(temp_rules_path) else generated_rules_path
...
def main():
    ...
    # check if parser_output.txt exists
    parser_output_path = os.path.join(traces_dir, "parser_output.txt")
    ...
    try:
        ...
        st.download_button(
            "Download LTS file",
            lts_content,
            os.path.join(lts_dir, "input_lts.aut"),
            "text/plain"
        )
    except Exception as e:
