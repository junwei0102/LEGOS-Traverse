...
legos_dir = os.path.join(project_root, "LEGOs")
...
def run_pipeline():
    ...
    if input_method == "Upload SLEEC File":
        ...
        if uploaded_file:
            ...
            # Save uploaded file
            sleec_file_path = os.path.join(rules_dir, "temp_rules.sleec")
    ...
    if sleec_content:
        ...
        if analysis_type == "Shared Responses":
            ...
            if selected_groups:  # use user selected groups instead of all groups
                ...
                st.success(f"Generated SLEEC file saved as '{generated_rules_path}'")
            ...
            if st.button("Generate Trace"):
                with st.spinner("Generating trace..."):
                    try:
                        ...
                        original_sleec_file = os.path.join(rules_dir, "temp_rules.sleec")
                        ...
                        if generate_lts_from_sleec(original_sleec_file, time_window, selected_rules):
                            ...
                            try:
                                lts_file_path = os.path.join(lts_dir, "input_lts.aut")
                                with open(lts_file_path, 'w') as f:
                                    f.write(lts)
