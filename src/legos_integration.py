...
sys.path.append(project_root)  # add project root directory
...
def run_sleec_parser(sleec_file: str, time_window: int = 15) -> Tuple[List[str], List[Dict]]:
    ...
    try:
        ...
        with open(parser_output_path, 'w') as f:
            f.write(output)
...
def generate_lts_from_sleec(sleec_file: str, time_window: int = 15, rule_ids: List[str] = None) -> bool:
    ...
    """
    try:
        # If sleec_file is a relative path, make it absolute
        if not os.path.isabs(sleec_file):
            sleec_file = os.path.join(rules_dir, sleec_file)
